<?php
    ...
    $ordenSQL = "select Fecha, (select Nombre from asignatura where ID=ID_Asignatura) as 'Asignatura' , (select concat(Nombre, ' ', Apellidos) from profesor where ID=ID_Profesor) as 'Profesor', amonestacion.Motivo from amonestacion where ID_Alumno='" . $id_ . "' and Fecha Between '".$fechaMin."' and '".$fechaMax."' and ID_Profesor='".$prof."' order by Fecha desc, Hora desc";
    $resultado = $conexion->query($ordenSQL);
    if ($resultado) {
        $fila = $resultado->fetch_array();
        while ($fila) {
    ...
    $ordenSQL = "select Fecha, (select Nombre from asignatura where ID=ID_Asignatura) as 'Asignatura' , (select concat(Nombre, ' ', Apellidos) from profesor where ID=ID_Profesor) as 'Profesor', expulsion.Motivo from expulsion where ID_Alumno='" . $id_ . "' and Fecha Between '".$fechaMin."' and '".$fechaMax."' and ID_Profesor='".$prof."' order by Fecha desc, Hora desc";
    $resultado = $conexion->query($ordenSQL);
    if ($resultado) {
        $fila = $resultado->fetch_array();
        while ($fila) {
    ...
    $ordenSQL = "select Fecha,  (select concat(Nombre, ' ', Apellidos) from profesor where ID=ID_Profesor) as 'Profesor', sancion.Sancion from sancion where ID_Alumno='".$id_."' and Fecha Between '".$fechaMin."' and '".$fechaMax."' and ID_Profesor='".$prof."' order by Fecha desc, Hora desc";
    $resultado = $conexion->query($ordenSQL);
    if ($resultado) {
        $fila = $resultado->fetch_array();
        while ($fila) {
